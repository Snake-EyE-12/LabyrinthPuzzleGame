...
public class ShowTile : Display<ShowTileData>
{
    ...
    public override void Render()
    {
        ...
        destinator.MoveTo(new DestinationData(new Vector3(item.pos.x * 2, item.pos.y * 2, 0), 0.0001f, false));
    }

    public void Move(Vector2Int newPos, bool instant)
    {
        if(instant) destinator.MoveTo(new DestinationData(new Vector3(newPos.x * 2, newPos.y * 2, 0), 0.0001f, false));
